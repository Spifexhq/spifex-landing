...
const OUTCOME_KEYS = ["visibility", "costing", "planning", "governance"] as const;
const STEP_KEYS = ["define", "move", "reconcile"] as const;
const FEATURE_KEYS = ["items", "movements", "valuation", "integrations"] as const;
const KPI_KEYS = ["stockAccuracy", "turnover", "shrinkage", "marginImpact"] as const;
    ...
    <main className="bg-white">
            ...
            <div className="max-w-3xl">
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                {HERO_BULLET_KEYS.map((k) => (
                  ...
                  >
                    {t(`inventoryPage.hero.bullets.${k}`)}
                  </span>
                ))}
              </div>
                ...
                <form action={DASHBOARD_URLS.signup} method="GET" className="w-full sm:max-w-md">
                  <div className="relative rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <input
                      ...
                      inputMode="email"
                      autoComplete="email"
                      placeholder={t("inventoryPage.hero.emailPlaceholder")}
                      className={[
        ...
        <Container>
          <div className="max-w-3xl">
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
              {t("inventoryPage.outcomes.title")}
            </h2>
            <p className="mt-3 text-slate-600">{t("inventoryPage.outcomes.subtitle")}</p>
          </div>
            ...
            {OUTCOME_KEYS.map((k) => (
              <div key={k} className="rounded-3xl border border-slate-200 bg-white p-6">
                <div className="text-sm font-semibold text-slate-900">
                  {t(`inventoryPage.outcomes.items.${k}.title`)}
                </div>
                <div className="mt-2 text-sm leading-relaxed text-slate-600">
                  {t(`inventoryPage.outcomes.items.${k}.desc`)}
                </div>
              </div>
            ))}
          </div>
        ...
        <Container>
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div className="max-w-xl">
              <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
                {t("inventoryPage.howItWorks.title")}
              </h2>
              <p className="mt-3 text-slate-600">{t("inventoryPage.howItWorks.subtitle")}</p>

              <div className="mt-8 space-y-4">
                {STEP_KEYS.map((k) => (
                  <div key={k} className="rounded-2xl border border-slate-200 bg-white p-5">
                    <div className="text-sm font-semibold text-slate-900">
                      {t(`inventoryPage.howItWorks.steps.${k}.title`)}
                    </div>
                    <div className="mt-2 text-sm leading-relaxed text-slate-600">
                      {t(`inventoryPage.howItWorks.steps.${k}.desc`)}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                <Button href="/pricing" variant="primary">
                  {t("inventoryPage.howItWorks.ctaPrimary")}
                </Button>
                <Button href={DASHBOARD_URLS.signup} variant="secondary">
                  {t("inventoryPage.howItWorks.ctaSecondary")}
                </Button>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-4">
              <div className="relative aspect-16/10 overflow-hidden rounded-2xl bg-slate-100">
                <Image
                  src="/products/inventory/inventory-ui.png"
                  alt={t("inventoryPage.howItWorks.imageAlt")}
                  fill
              ...
              <div className="mt-3 px-2 text-xs text-slate-500">
                {t("inventoryPage.howItWorks.imageCaption")}
              </div>
            </div>
          </div>
        </Container>
      ...
      <section className="py-14 sm:py-18">
        <Container>
          <div className="max-w-3xl">
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
              {t("inventoryPage.features.title")}
            </h2>
            <p className="mt-3 text-slate-600">{t("inventoryPage.features.subtitle")}</p>
          </div>
            ...
            {FEATURE_KEYS.map((k) => (
              <div key={k} className="rounded-3xl border border-slate-200 bg-white p-7">
                <div className="text-xl font-semibold tracking-tight text-slate-900">
                  {t(`inventoryPage.features.items.${k}.title`)}
                </div>
                <div className="mt-3 text-sm leading-relaxed text-slate-600">
                  {t(`inventoryPage.features.items.${k}.desc`)}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="bg-white py-14 sm:py-18">
        <Container>
          <div className="rounded-3xl border border-slate-200 bg-white p-8 sm:p-10">
            <div className="max-w-3xl">
              <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
                {t("inventoryPage.kpis.title")}
              </h2>
              <p className="mt-3 text-slate-600">{t("inventoryPage.kpis.subtitle")}</p>
            </div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {KPI_KEYS.map((k) => (
                <div key={k} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                  <div className="text-sm font-semibold text-slate-900">
                    {t(`inventoryPage.kpis.items.${k}.title`)}
                  </div>
                  <div className="mt-2 text-sm leading-relaxed text-slate-600">
                    {t(`inventoryPage.kpis.items.${k}.desc`)}
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="bg-slate-50 py-14 sm:py-18">
        <Container>
          <div className="rounded-3xl border border-slate-200 bg-white px-6 py-12 text-center sm:px-12 sm:py-14">
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
              {t("inventoryPage.bottom.title")}
            </h2>
            <p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-slate-600">
              {t("inventoryPage.bottom.subtitle")}
            </p>

            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button href="/pricing" variant="primary">
                {t("inventoryPage.bottom.ctaPricing")}
              </Button>
              ...
              >
                {t("inventoryPage.bottom.ctaSignup")}
              </Link>
            </div>
          </div>
        </Container>
